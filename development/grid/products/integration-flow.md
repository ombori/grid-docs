...
3. **Access Token**
  - You need to generate an Access Token under "Developers" tab to be used later on for every request
     - [Products API](/development/grid/products/api?id=request-authentication)
     - [Spaces API](/development/grid/products/spaces-api?id=request-authentication)

## 2. Push Spaces

This step is applicable if you want your data to be visible only to a specific store / location for example.

To Read about how to create spaces into the database using our API, check the [Create Space endpoint](/development/grid/products/api?id=post-space) in the API reference.

The returned `id` value will be used as values for `spaceIds` and `spaceId` fields later on when you format your product data into the [GridProduct](/development/grid/products/data-model?id=gridproduct) format
...
To Read about how to push product types into the database using our API, check the [Push Product Types endpoint](/development/grid/products/api?id=post-push-product-types) in the API reference.
...
If you want to integrate it in your own backend using our JavaScript npm package, proceed with the [Integration Setup](/development/grid/products/integration-flow?id=integration-setup) section of this guide.

**Integrate using APIs**

If you want to integrate using our API from any other system, you can check the [API Reference](/development/grid/products/api), we have a Postman collection you can import into [Postman](https://www.postman.com/) and explore the API as well.

To read about how to upload products in the database using our API, check the  [Push Products endpoint](/development/grid/products/api?id=post-push-products) in the API reference.
...
Pushing products into the database is limited, for accurate limitations see the `Push Products` section in the [API reference](/development/grid/products/api?id=post-push-products).
